...
mcp = FastMCP(
    "OpenAIImageGen", # Internal name, might be overridden by settings key
...
async def generate_image(
    ...
    # background: Optional[Literal["transparent", "opaque"]] = "opaque", # Background options - REMOVED
    # moderation: Optional[Literal["auto", "low"]] = "auto", # Moderation strictness - REMOVED
    # response_format is implicitly b64_json based on example, format param controls file type if saved
    # output_compression is only for jpeg/webp, not directly controlled here yet
    ...
    Args:
        ...
        # background: Background ('transparent' or 'opaque'). Default: 'opaque'. (REMOVED - Parameter not supported by API)
        # moderation: Content moderation strictness ('auto' or 'low'). Default: 'auto'. (REMOVED - Parameter not supported by API)
    ...
    """
    logging.info(f"Tool 'generate_image' called with prompt: '{prompt[:50]}...'") # Removed background reference from initial log
    ...
    try:
        logging.info(f"Requesting image generation from OpenAI with model={model}, size={size}, quality={quality}, n={n}") # Removed background & moderation from log
        ...
        api_args = {
            ...
            "size": size,
            "quality": quality,
            # "background": background, # Removed based on error
            # "moderation": moderation, # Removed based on error
            "user": user
            # "response_format": "b64_json" # Removed based on error
        }
        ...
        try:
            ...
            save_dir = os.path.join(script_dir, "ai-images")

            # Ensure directory exists
            os.makedirs(save_dir, exist_ok=True)
            full_save_path = os.path.join(save_dir, final_filename) # Now an absolute path
...
async def edit_image(
    ...
    try:
        ...
        logging.info(f"Requesting image edit from OpenAI with model={model}, size={size}, quality={quality}, n={n}") # Removed background & moderation from log
        ...
        except Exception as save_e: # Corrected indentation
            logging.error(f"Failed to save edited image: {save_e}") # Corrected indentation
            # Return failure message if saving failed
            return {"status": "error", "message": f"Image edited but failed to save: {save_e}"} # Corrected indentation
        # --- End: Force Save Logic ---
...
# --- Main execution block (for stdio transport via python command) ---
if __name__ == "__main__":
    logging.info("Starting OpenAI Image Generation MCP server (stdio)...")
    # This allows running the server directly using 'python openai_image_mcp.py'
    # It will use the default stdio transport.
    try:
        mcp.run() # This handles the stdio communication loop
